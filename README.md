...
以倒计时数字为视觉中心的极简效率工具。v0.2.0 全面升级为**多任务多窗口**架构：支持同时运行多个时长倒计时与日期/纪念日倒计时，每个任务可单独打开悬浮窗、极简窗或全屏模式；新增年度/月度/今日进度、记录自动推送 Git。所有数据仅保存在本地。
...
产物位于 `release/` 目录：`桌面倒计时-Setup-0.11.2.exe`。安装后无需 Node.js 环境，可独立运行。
...
- 新建表单模式排布调整：时长倒计时与定点倒计时位置互换，时长倒计时回到第一位并默认选中

### v0.11.1 变更

- **铃声频率提升**：到点后 30 秒内连续提醒 **30 次**（每 1 秒一次双音铃声，原为 20 次/1.5 秒间隔）；仍可横幅一键停止
