...
const cards = computed(() => [
  { key: 'year', icon: '📅', title: `${now.value.getFullYear()} 年度进度`, p: year.value, note: `已过 ${elapsedDays(year.value)} · 剩余 ${remainDays(year.value)}` },
  { key: 'month', icon: '🌙', title: `${now.value.getMonth() + 1} 月月度进度`, p: month.value, note: `已过 ${Math.floor(month.value.elapsedMs / 86_400_000)} 天 · 剩余 ${remainDays(month.value)}` },
  { key: 'day', icon: '☀️', title: '今日进度', p: day.value, note: `剩余 ${remainToday(day.value)}` }
])
</script>

<template>
  <div class="progress-page">
    <div class="date-line">{{ formatCNDate(now) }} · {{ weekText }}</div>
    <div v-for="c in cards" :key="c.key" class="pcard card">
      ...
      <ProgressBar :percent="c.p.percent" />
